    ...
    <AccordionPanel pb={4}>
      ...
      <VStack spacing={"10px"} width={"100%"} textAlign="start" fontWeight={"bold"} fontSize={"12px"}>
        ...
        <HStack width={"100%"} justifyContent="space-between"><Text>GET 30% OFF USING PHARM30</Text><Radio value='3'></Radio></HStack>
            ...
            <Button disabled={cartItems.length < 1 } width={"100%"} onClick={()=>navigate("/delivery")} colorScheme={"teal"}>Tiến hành mua</Button>
            <Divider/>
            <VStack width={"100%"} >
            <Text width={"100%"} textAlign={"left"} fontWeight="500"> Tóm tắt giỏ hàng</Text>
            <HStack width={"100%"} justify={"space-between"}><Text fontSize={"14px"}>Giá trị giỏ hàng</Text><Text><span style={{marginRight:"10px",fontSize:"12px",textDecoration:"line-through"}}>{totalOriginalAmount}</span> {totalAmount} VND</Text></HStack>
            <HStack width={"100%"} justify={"space-between"}><Text fontSize={"14px"}>Số tiền phải trả</Text><Text>{totalAmount} VNĐ</Text></HStack>
            </VStack>
